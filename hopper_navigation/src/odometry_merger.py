...
class OdometryMerger(Thread):
    def __init__(self):
        ...
        self._slam_subscriber = rospy.Subscriber("", Odometry, self._on_slam_odometry)
        self._odometry_subscriber = rospy.Subscriber("Odom", Odometry, self._on_robot_odometry)
        self._merged_odometry_publisher = rospy.Publisher("merged_odometry", Odometry, queue_size=10)
