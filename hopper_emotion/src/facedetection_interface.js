...
if (isLinux) {
    ...
    rosnodejs.initNode('hopper_web_emotion_interface')
        .then((node) => {
            // hopperFaceSubscriber = node.subscribe('/detected_faces', 'hopper_emotion/FaceDetectionImage', faceDetectionHandler);
            hopperFaceSubscriber = node.subscribe('/identified_faces', 'hopper_emotion/FaceDetectionImage', faceDetectionHandler);
        });
}
